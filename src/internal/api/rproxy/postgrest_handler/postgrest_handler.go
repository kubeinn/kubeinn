...
func PostgrestHandler(c *gin.Context) {
	...
	if strings.HasPrefix(c.Request.URL.Path, global.API_ROUTE_PREFIX+global.INNKEEPER_ROUTE_PREFIX) {
		...
		path = strings.TrimPrefix(c.Request.URL.Path, global.API_ROUTE_PREFIX+global.INNKEEPER_ROUTE_PREFIX)
		...
		path = strings.TrimPrefix(c.Request.URL.Path, global.API_ROUTE_PREFIX+global.PILGRIM_ROUTE_PREFIX)
	}
	subject = c.Request.Header.Get("subject")

	// Format request for Postgrest
